...
In this context, a **critical impact rainfall threshold** is defined as the precipitation level that causes unsustainable or unacceptable impacts. This concept is closely related to the **"risk-tolerance level"**, defined as the maximum amount of damage, impact, or frequency that areas, communities, and regions are prepared to handle due to climate hazards (in this case, extreme precipitation). 

```{note}
The thresholds and risk tolerance levels will naturally vary across communities and sectors (e.g., transportation or agriculture) as risk thresholds are not absolute values but a social construct that depends on the local context, the coping capacity, and risk perception.
```

![Extreme precipitation illustration](https://github.com/CLIMAAX/HEAVY_RAINFALL/blob/main/images/Extreme_precipitation_Impact_pyramid.png?raw=true "Extreme precipitation illustration")
...
## Guide to developing impact-based rainfall thresholds
...
add some explanation on the figure below
</span>
